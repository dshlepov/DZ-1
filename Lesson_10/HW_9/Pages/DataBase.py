...
class DataBase:
    ...
    def create_company(self, company_name: str, description: str):
        try:
            with self.db.connect() as connection:
                result = connection.execute(self.query['create_company'],{'name': company_name, 'description': description})
                connection.commit()
                return result
    ...
    def delete(self, company_id: int):
        try:
            with self.db.connect() as connection:
                connection.execute(self.query['delete_company'], {'company_id': company_id})
                connection.commit()
        except Exception as _ex:
            print("[INFO] Error - can't work with SQL", _ex)
        finally:
    ...
    @allure.step("Получаем ID последней созданной помпании")
    ...
    def gett_list_employer(self, company_id: int):
        try:
            with self.db.connect() as connection:
                result = connection.execute(self.query['list_SELECT'], {'id': company_id}).fetchall()
                return result
        except Exception as _ex:
            print("[INFO] Error - can't work with SQL", _ex)
        finally:
    ...
    def create_employer(self, company_id: int, first_name: str, last_name: str, phone: str):
        try:
            with self.db.connect() as connection:
                result = connection.execute(self.query['item_INSERT'], {'id': company_id, 'name': first_name, 'surname': last_name,
                                                                                    'phone_num': phone})
                connection.commit()
                return result
        except Exception as _ex:
    ...
    def get_employer_id(self, company_id: int):
        try:
            with self.db.connect() as connection:
                result = connection.execute(self.query['maxID_SELECT'], {'c_id': company_id}).fetchall()
                if result:
                    return result[0][0]
            return None
        except Exception as _ex:
    ...
    def update_employer_info(self, new_name: str, id: int):
        try:
            with self.db.connect() as connection:
                result = connection.execute(self.query['item_UPDATE'], {'new_name': new_name, 'employer_id': id})
                connection.commit()
        except Exception as _ex:
            print("[INFO] Error - can't work with SQL", _ex)
        finally:
